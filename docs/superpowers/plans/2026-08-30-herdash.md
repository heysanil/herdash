...
| `src/summary/types.rs` | `AgentSummary`, `SummarySlot`, sanitization |
...
| `src/ui/theme.rs` | Colors, glyphs, status styling |
...
| `tests/policy.rs` | Summarization decision table |
| `tests/summary_parse.rs` | Structured-output parsing and sanitization |
...
impl AgentStatus {
    /// Map a wire string to a status. Unrecognized values become [`Self::Unknown`]
...
## Task 4: Summarization policy (pure decision function)
...
git commit -m "feat: add pure summarization cadence policy with bypasses and backoff"
```

---

## Task 5: OpenRouter summarizer
...
git commit -m "feat: add OpenRouter summarizer with strict structured outputs"
```
...
fn an_in_flight_summary_shows_progress() {
    ...
    let text = render(&a, 120, 40).join("\n");
    assert!(text.contains("summarizing"));
}
...
```rust
//! Colors, glyphs and layout constants.
//!
...
fn secondary(app: &App, a: &Agent, width: usize) -> Vec<String> {
    ...
    if let Some(slot) = slot {
        ...
        if slot.state.in_flight {
            return vec!["summarizing…".to_string()];
        }
...
fn the_help_overlay_documents_every_binding() {
    ...
    a.show_help = true;
    let text = render(&a, 140, 40);
    for hint in ["select", "focus pane", "resummarize", "active", "quit"] {
...
## Task 10: Event loop, connection task, reconnect and summarization orchestration
...
//! summarizer tasks are spawned per agent; the main loop multiplexes crossterm
...
async fn summarize_one(
    ...
    if transcript.trim().is_empty() {
        anyhow::bail!("pane produced no output to summarize");
    }
...
git commit -m "feat: wire the event loop, connection task and summarizer dispatch"
```
...
git commit -m "docs: add README, AGENTS.md and license"
```
